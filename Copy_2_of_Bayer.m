...
output = 0;
ChannelX = zeros(height, width);
ChannelY = zeros(height, width);
...
%         BayerMatrix4 = 16 + BayerMatrix3;
% 
%         if (Sample(i,j)/4 >= 0 && Sample(i,j)/4 < 16)
%             if (Sample(i,j)/4 > BayerMatrix1) output(i,j) = 63;
%             else output(i,j) = 0;
%             end
%         elseif (Sample(i,j)/4 > 15 && Sample(i,j)/4 < 32)
%             if (Sample(i,j)/4 >= BayerMatrix2) output(i,j) = 127;
%             else output(i,j) = 63;
%             end
%         elseif (Sample(i,j)/4 > 31 && Sample(i,j)/4 < 48)
%             if (Sample(i,j)/4 >= BayerMatrix3) output(i,j) = 191;
%             else output(i,j) = 127;
%             end
%         else
%             if (Sample(i,j)/4 >= BayerMatrix4) output(i,j) = 255;
%             else output(i,j) = 191;
%             end
%         end
%     end
% end
% ImageSample = uint8(output);
% imwrite(ImageSample, './sample/4StepSample.png');
